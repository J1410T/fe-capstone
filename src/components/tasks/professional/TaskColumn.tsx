...
export const TaskColumn: React.FC<TaskColumnProps> = ({ status, tasks, onTaskClick }) => {
  ...
  return (
    <div className={`flex flex-col h-full rounded-lg border-2 ${config.borderColor} ${config.bgColor} ${
      isOver ? "ring-2 ring-blue-400 ring-opacity-50" : ""
    }`}>
      {/* Column Header */}
      <div className={`${config.headerColor} px-4 py-3 rounded-t-lg border-b ${config.borderColor}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <Icon className={`w-4 h-4 ${config.textColor}`} />
            <h3 className={`font-semibold text-sm ${config.textColor}`}>{status}</h3>
          </div>
